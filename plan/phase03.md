...
## 八、G03 验收记录（2026-07-11，phase03 分支）

| # | Gate 标准 | 结果 | 证据 |
|---|---|---|---|
| 1 | 月报能力：M06 端到端签发，MoM+YoY 审计 100% | ✅ | run #29 命中 `treasury-monthly`/2026-M06，环比基期 M05、同比基期 2025-M06 程序推导；`_mom`×7 + `_yoy`×6（工资同比基数 0 正确跳过留 note）；审计 42/42=100%，PUBLISHED；演示 11 实测翻转 |
| 2 | 季报能力：Q2 端到端签发，QoQ+YoY 审计 100% | ✅ | run #30 命中 `treasury-quarterly`/2026-Q2（环比 Q1、同比 2025-Q2）；`_qoq`×7 + `_yoy`×7；审计 45/45=100%，PUBLISHED |
| 3 | 维度能力：周报按币种拆解，逐格 100% | ✅ | treasury-weekly v3 加 `by_currency` 章；run #31 表格逐格 `{{fact_key}}`（`fact_017_cny`/`_usd` + 合计 + `_share`），`dimensions_json` 查库非空；审计 43/43=100%，PUBLISHED |
| 4 | 评测扩充后全量通过、不低于 Phase02 基线 | ✅ | golden-set 扩至 15 MATCH + 6 BLOCKED + 7 FACTS（100 条期望值，53 条新增全部手写 SQL 直查；b02 与月报能力同批翻转、年报接任反例）；确定性层 **100/100=100%**（47 条存量原样通过）、匹配 **15/15**、失败关闭 **6/6**——README「评测基线」节已更新 |
| 5 | 零回退 | ✅ | `mvn test` **337/337**（294 存量 + 43 新增）；W26 基线扩展而非替换（run #31 即含维度章的 W26 周报，43/43）；对抗演练「季报周期塞给周报模板」→ `[POLICY] 模板「司库资金周报」不支持 QUARTER 粒度…` 服务端矩阵把关命中 |

**实施发现**（记入说明书 12.3 候选）：① 快照口径指标（`in_transit_txn_count` 等）不带周期过滤，增量种子任何非 SETTLED 行都会击穿既有期望值——已在 db/00 增量段注释固化教训；② 维度指标本阶段禁 `comparable`（维度行不做环比/同比，`MetricDimensionRule` 拦截）——维度×比较的交叉留待归因阶段按需开闸；③ 合计 fact 恒由程序对行求和（不试图「引用同章总量指标」——机器无法判定口径同源性，独立求和保确定性）；④ 运维教训：多应用实例并存时 8080 由旧实例持有，评测打到旧代码产生假回退——验证前须确保单实例。
