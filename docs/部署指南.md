...
把源码里的 `db/` 目录上传到服务器（下一步 7.5.1 一并上传），然后**按编号顺序**执行四个脚本：
...
# mysql -uroot -p reportbi < db/04-business-data-v2.sql  # 业务数据扩容 v2（2024-01~2026-08 逐月、5 币种、26 个账户）
...
> `04-business-data-v2.sql` 是**纯增量**脚本、行都带显式主键：只能紧跟 `00-init.sql` 之后跑一次，
> 重复执行会报主键冲突（这是刻意的——防止把演示数据灌成两倍）。
