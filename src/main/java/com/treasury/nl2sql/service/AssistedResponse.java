...
public record AssistedResponse(
    ...
    public static AssistedResponse hit(long assetId, String matchedQuestion, double score, NlQueryResult trace) {
        return new AssistedResponse(QueryState.HIT, "CALIBER", score, assetId, matchedQuestion, null, trace);
    }

    public static AssistedResponse generated(NlQueryResult trace) {
        return new AssistedResponse(QueryState.GENERATED, "LLM", 0, null, null, null, trace);
    }

    public static AssistedResponse failed(NlQueryResult trace) {
        return new AssistedResponse(QueryState.FAILED, "LLM", 0, null, null, null, trace);
    }

    /** 候选口径待确认（召回落在中间档）。 */
    public static AssistedResponse clarifyCandidate(long assetId, String matchedQuestion, double score, String prompt) {
        return new AssistedResponse(QueryState.CLARIFY, "CALIBER", score, assetId, matchedQuestion, prompt, null);
    }

    /** 模型拒答，需用户补充后重问。 */
    public static AssistedResponse clarifyReject(String prompt, NlQueryResult trace) {
        return new AssistedResponse(QueryState.CLARIFY, "LLM", 0, null, null, prompt, trace);
    }
}
